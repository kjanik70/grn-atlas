...
async def find_paths(
    ...
    """
    source = db.get_gene(source_gene_id)
    target_results = db.search_genes(target_symbol, limit=1)
    ...
    while queue and len(paths) < limit:
        ...
        if len(current_path) < max_depth:
            targets = db.get_targets(current_id, min_confidence)
            for target in targets:
                if target.id not in visited and target.regulation_type in regulation_type:
